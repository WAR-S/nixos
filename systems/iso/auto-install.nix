...
let
  # Скрипт самоустановки: читает cmdline, при наличии nixos.autoInstall=1 ставит систему на nixos.installDisk
  autoInstallScript = pkgs.writeScript "nixos-auto-install" ''
    #!${pkgs.runtimeShell} -e
    set -e
    export PATH="${pkgs.lib.makeBinPath [ pkgs.gnugrep pkgs.util-linux pkgs.nix pkgs.coreutils diskoPackage ]}"
